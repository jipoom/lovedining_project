...
@section('content')
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="#tab-content" data-toggle="tab" onclick="mkDir()">Review Content</a></li>
		</ul>
				...
				<div class="form-group {{{ $errors->has('title') ? 'error' : '' }}}">
                    <div class="col-md-12">
					    ...
					    {{ Form::text('zip', Input::old('title', isset($post) ? $post->zip : null), array('placeholder'=>'รหัสไปรษณีย์')) }} </p>  
                         ...
                         {{ Form::hidden('temp_album_name', Input::old('temp_album_name', isset($post) ? $post->temp_album_name : "null"), array('id'=>'temp_album_name')) }} </p>  
     					 {{ Form::hidden('review_id', Input::old('review_id', isset($post) ? $post->id : "null"), array('id'=>'review_id')) }} </p>  
	...
	function mkDir() {
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				document.getElementById("post").val = xmlhttp.responseText;
			}
		}
		xmlhttp.open("GET", "{{ URL::to('admin/blogs/makeDir/') }}/"+ id+"/"+newDir , true);
		xmlhttp.send();
