...
class PostsUserRead extends Eloquent 
{
	...
	public static function getUnreadReviews($category,$userId)
	{
		...
		$diff = Post::getReviewsByCategory(null,$category -> id,'created_at','DESC')->count() - PostsUserRead::join('posts_category','posts_user_read.post_id','=','posts_category.post_id')->where('user_id', '=', $userId) -> where('posts_category.category_id', '=', $category -> id) -> count();
		return $diff;
